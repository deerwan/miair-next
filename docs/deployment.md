...
> ⚠️ **凭据存储**: `conf/config.json` 以明文保存小米账号密码 / Cookie，`conf/.mi.token` 保存 serviceToken。程序在每次保存配置与登录成功后会自动把这两个文件的权限收紧为 `600`（仅属主可读写，`secret.key` 亦同），但仍请妥善保护数据目录：不要放在公开可读的共享路径，备份文件注意加密存放。
